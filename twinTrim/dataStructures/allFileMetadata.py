...
def add_or_update_file(file_path: str):
    ...
    with store_lock:
        ...
        if existing_file_metadata is None:
            ...
            # Use compare_and_replace and update store reference if file is replaced
            existing_file_metadata.compare_and_replace(new_file_metadata)
            # Ensure store reference is updated to the latest AllFileMetadata
            store[file_hash] = existing_file_metadata
